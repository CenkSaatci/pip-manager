...
export function DiceRollerPanel({ char, rules, onCharChange }: { char: Character; rules: RuleSet; onCharChange?: (c: Character) => void }) {
  ...
  return (
    <div className="pip-panel rounded-sm p-4">
      <h3 className="pip-label mb-3">Würfelterminal (W10-Pool, roll-under)</h3>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        ...
        >
          ...
          {weaponsForSkill.map((w) => (
            <option key={w.id} value={w.id}>
              {w.name} (Schaden {w.damage})
            </option>
          ))}
        </select>
        {weapon?.isAutomatic && (
        ...
        <select value={difficultyName} onChange={(e) => setDifficultyName(e.target.value)} className="pip-input rounded-sm px-2 py-1">
          {rules.difficultyLevels.map((d) => (
            <option key={d.name} value={d.name}>
              {d.name} (Malus {d.penalty}, {d.successesRequired} Erfolge)
            </option>
          ))}
        </select>
        <select value={coverPenalty} onChange={(e) => setCoverPenalty(Number(e.target.value))} className="pip-input rounded-sm px-2 py-1">
          ...
          <option value={-1}>Leichte Deckung (-1)</option>
          <option value={-2}>Gute Deckung (-2)</option>
        </select>
        <select value={targetZone} onChange={(e) => setTargetZone(e.target.value)} className="pip-input rounded-sm px-2 py-1">
          ...
          {rules.hitLocations.map((h) => (
            <option key={h.id} value={h.id}>
              {h.name} ({h.penalty})
            </option>
          ))}
        </select>
        <button onClick={doRoll} className="pip-btn">
          Würfeln
        </button>
      </div>
      ...
      {skill && (
        <p className="mb-2 text-xs text-pip-greendim">
          Würfelpool: {getDicePoolSize(skill, char, rules)} W10 (Skill {getSkillEffectiveValue(skill, char, rules)} +
          Luck-Bonus) · Zielwert vor Modifikatoren: {getSkillEffectiveValue(skill, char, rules)}
        </p>
      )}
      ...
      {lastResult && (
        <div className="mb-4 rounded-sm border border-pip-line p-2 text-sm">
          <p>
            Zielwert: <span className="text-pip-amber">{lastResult.target}</span> · Würfe ({lastResult.poolSize}):{" "}
            {lastResult.rolls.join(", ")} · Erfolge: {lastResult.successes}
            {lastResult.naturalOnes > 0 ? ` (davon ${lastResult.naturalOnes}× natürliche 1)` : ""}
          </p>
          <p className={lastResult.passed ? "text-pip-green text-glow" : "text-pip-red"}>
            {lastResult.passed ? "ERFOLG" : "FEHLSCHLAG"}
            {zone ? ` · Zielzone: ${zone.name}` : ""}
          </p>
          {lastDamage !== null && (
            <p className="mt-1 text-pip-amber">
              Schaden = {weapon?.damage} (Waffe) + {lastResult.successes} (Erfolge)
              {burstFire && weapon?.isAutomatic ? ` + ${weapon.burstDamageBonus ?? 0} (Dauerfeuer)` : ""} ={" "}
              <strong>{lastDamage}</strong>
              {lastResult.naturalOnes > 0 ? " · +1 durch natürliche 1 möglich (SL-Entscheid)" : ""}
            </p>
          )}
        </div>
      )}
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 border-t border-pip-line pt-3">
        ...
        <input
          ...
          min={0}
          value={armorDr}
          onChange={(e) => setArmorDr(Number(e.target.value))}
          className="pip-input w-16 rounded-sm px-2 py-1 text-center"
        />
        <button onClick={doArmorCheck} className="pip-btn-ghost">
          1W10 gegen DR würfeln
        </button>
        {armorResult && (
          <span className={armorResult.blocked ? "text-pip-green text-glow" : "text-pip-red"}>
            Wurf {armorResult.roll} — {armorResult.blocked ? "Schaden geblockt" : "voller Schaden"}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-pip-line pt-3">
        ...
        <button onClick={doInitiative} className="pip-btn-ghost">
          Sequence würfeln (PER + 1W10)
        </button>
        {initiative && (
